...
class LocaltuyaVacuum(LocalTuyaEntity, StateVacuumEntity):
    ...
    async def async_start(self, **kwargs):
        ...
        await self._device.set_dp(True, self._config[CONF_POWERGO_DP], self._cid)

    async def async_pause(self, **kwargs):
        """Stop the vacuum cleaner, do not return to base."""
        await self._device.set_dp(False, self._config[CONF_POWERGO_DP], self._cid)

    async def async_return_to_base(self, **kwargs):
        """Set the vacuum cleaner to return to the dock."""
        if self.has_config(CONF_RETURN_MODE):
            await self._device.set_dp(
                self._config[CONF_RETURN_MODE], self._config[CONF_MODE_DP], self._cid
            )
        else:
    ...
    async def async_stop(self, **kwargs):
        ...
        if self.has_config(CONF_STOP_STATUS):
            await self._device.set_dp(
                self._config[CONF_STOP_STATUS], self._config[CONF_MODE_DP], self._cid
            )
        else:
    ...
    async def async_locate(self, **kwargs):
        ...
        if self.has_config(CONF_LOCATE_DP):
            await self._device.set_dp("", self._config[CONF_LOCATE_DP], self._cid)

    async def async_set_fan_speed(self, fan_speed, **kwargs):
        """Set the fan speed."""
        await self._device.set_dp(fan_speed, self._config[CONF_FAN_SPEED_DP], self._cid)

    async def async_send_command(self, command, params=None, **kwargs):
        """Send a command to a vacuum cleaner."""
        if command == "set_mode" and "mode" in params:
            mode = params["mode"]
            await self._device.set_dp(mode, self._config[CONF_MODE_DP], self._cid)
