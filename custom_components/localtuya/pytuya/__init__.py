...
class TuyaProtocol(asyncio.Protocol, ContextualLogger):
    ...
    def _setup_dispatcher(self):
        def _status_update(msg):
            ...
            if "dps" in decoded_message and decoded_message["cid"] == self.id:
    ...
    async def exchange(self, command, cid=None, dps=None):
        ...
        payload = self._generate_payload(command, cid, dps)
        ...
        if dev_type != self.dev_type:
            ...
            return await self.exchange(command, cid, dps)
        return payload
    ...
    async def set_dp(self, value, dp_index, cid):
        """
        ...
        """
        return await self.exchange(SET, cid, {str(dp_index): value})

    async def set_dps(self, dps, cid):
        """Set values for a set of datapoints."""
        return await self.exchange(SET, cid, dps)
    ...
    def _generate_payload(self, command, cid, data=None):
        """
        ...
        if cid != None:
            ...
            if "devId" in json_data:
                json_data["devId"] = cid
            if "uid" in json_data:
                json_data["uid"] = cid  # still use id, no separate uid
        else:
            if "gwId" in json_data:
