...
def get_entity_config(config_entry, dp_id,cid):
    ...
    for entity in config_entry.data[CONF_ENTITIES]:
        if entity[CONF_ID] == dp_id and entity[CONF_CID_STRING] == cid:
            return entity
    raise Exception(f"missing entity config for id {dp_id} cid {cid}")
...
class TuyaDevice(pytuya.TuyaListener, pytuya.ContextualLogger):
    ...
    async def set_dp(self, state, dp_index, cid):
    ...
    async def set_dps(self, states, cid):
...
class LocalTuyaEntity(RestoreEntity, pytuya.ContextualLogger):
    ...
    def __init__(self, device, config_entry, dp_id , cid, logger, **kwargs):
        """Initialize the Tuya entity."""
        super().__init__()
        self._device = device
        self._config_entry = config_entry
        self._config = get_entity_config(config_entry, dp_id, cid)
        self._dp_id = dp_id
        self._cid = cid
        self._status = {}
    ...
    def unique_id(self):
        ...
        return f"local_{self._config_entry.data[CONF_DEVICE_ID]}_{self._cid}_{self._dp_id}"
